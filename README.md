...
v1.0.1-beta 是实验分支，当前重点用于验证完整牌堆、角色版开局、卡牌效果和回合流程。

该版本仍有较多卡牌逻辑和流程问题需要后续修复，尤其是角色技能、黑牌触发、审判牌效果、传染结算、自首/保护公布、夜晚与黎明阶段衔接、死亡与身份翻出后的后续流程。当前不要把 beta 版作为正式规则依据，实际开局建议继续使用稳定的 v1.0.1 正式版。
